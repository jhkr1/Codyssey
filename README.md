...
## 15. 컨테이너 생명주기와 접속 방식 (attach vs exec)

컨테이너를 다루다 보면 "분명히 실행해둔 컨테이너가 접속을 끊었더니 갑자기 종료되는" 현상을 겪게 됩니다. 이를 방지하고 컨테이너를 안전하게 운영하기 위해 두 접속 방식의 차이를 검증 및 정리합니다.

### 핵심 대원칙: "컨테이너의 생명 = 메인 프로세스(PID 1)의 생명"
컨테이너는 완전한 가상 머신(VM)이 아니라, 특정 프로그램을 실행하기 위한 격리된 껍데기입니다. 따라서 컨테이너가 실행될 때 최초로 구동된 **메인 프로세스(PID 1)가 종료되면, 컨테이너도 자신의 역할을 다했다고 판단하여 즉시 종료(Exited) 상태로 전환**됩니다.

### 1) docker attach (메인 프로세스 직접 연결)
* **개념:** 현재 실행 중인 컨테이너의 **메인 프로세스**에 내 터미널 화면을 직접 연결(Attach)합니다.
* **특징:** 컨테이너가 뿜어내는 실시간 로그나 프로세스의 메인 출력을 모니터링할 때 유용합니다.
* **주의점 :** 이 상태에서 터미널에 `exit`를 치거나 `Ctrl + C`를 누르면 종료 시그널이 메인 프로세스에 그대로 전달되어 **컨테이너 전체가 멈추게 됩니다.**
* **안전한 탈출법:** 컨테이너를 죽이지 않고 화면만 빠져나오려면(Detach), `Ctrl + P`를 누른 직후 `Ctrl + Q`를 입력해야 합니다.

### 2) docker exec (새로운 프로세스 우회 접속)
* **개념:** 이미 가동 중인 컨테이너 내부에 **새로운 쉘(Shell)이나 프로세스를 추가로 생성**하여 접속합니다. (예: `docker exec -it <컨테이너명> bash`)
* **특징:** 컨테이너 내부 환경을 점검하거나, 패키지 설치, 파일 수정을 할 때 사용하는 **실무 표준 접속 방식**입니다.
* **안전성 :** 뒷문을 열고 들어가는 것과 같아서, 작업을 마치고 `exit`를 입력해도 방금 연 '새로운 프로세스'만 종료될 뿐, 메인 프로세스는 영향을 받지 않으므로 **컨테이너가 계속 실행(Up) 상태를 유지**합니다.

### 요약 비교표

| 구분 | `docker attach` | `docker exec` |
| :--- | :--- | :--- |
| **접속 대상** | 컨테이너의 **메인 프로세스** (PID 1) | 컨테이너 내부에 생성한 **새로운 프로세스** |
| **주 사용 목적** | 실시간 프로세스 출력/로그 모니터링 | 내부 점검, 환경 설정, 추가 명령어 실행 |
| **`exit` 입력 시** | **컨테이너 종료됨 (Stop)** | **컨테이너 유지됨 (Up)** |

###  본 프로젝트 실습 적용 사례
본 프로젝트의 **볼륨 영속성 검증(11번 항목)**에서 컨테이너를 띄울 때 `sleep infinity`(무한 대기)를 메인 프로세스로 실행해 두었습니다. 이후 컨테이너 내부에 들어가 파일을 생성하고 확인할 때는 `docker exec`를 사용하여 우회 접속했습니다. 이 구조 덕분에 내부에서 작업을 마치고 빠져나와도 컨테이너가 죽지 않고 백그라운드에서 안전하게 유지될 수 있었습니다.
